...
echo "[gofractal] bitcoind entrypoint starting"

if ! command -v bitcoind >/dev/null 2>&1; then
  echo "[gofractal] ERROR: bitcoind not found in PATH"
  exit 127
fi

extra=""
if [ -f /data/.reindex-chainstate ]; then
  echo "[gofractal] Reindex requested (chainstate)."
...
echo "[gofractal] Using dbcache=${dbcache}MB"
echo "[gofractal] Exec: bitcoind -datadir=/data -printtoconsole -dbcache=${dbcache} $extra"
